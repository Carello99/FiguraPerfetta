...
export class TriangoloPage implements AfterViewInit{
  ...
  */
  calcolaPrecisione() {
    ...
    // Calcola il perimetro del triangolo ideale che può contenere il disegno
    const perimetroTriangoloIdeale = this.calcolaPerimetroTriangoloIdeale();
    ...
    this.precisione = Math.min(Math.floor((percorsoCampione / perimetroTriangoloIdeale) * 100), 100);
    this.precisioneCalcolata = true;
  
    console.log('Precisione Triangolo:', this.precisione, '%');
  }
  
  private calcolaPerimetroTriangoloIdeale(): number {
    const puntiEstremi = this.trovaPuntiEstremi();
    const perimetro = this.calcolaDistanza(puntiEstremi[0], puntiEstremi[1]) +
                      this.calcolaDistanza(puntiEstremi[1], puntiEstremi[2]) +
                      this.calcolaDistanza(puntiEstremi[2], puntiEstremi[0]);
    return perimetro;
  }
  
  private trovaPuntiEstremi(): { x: number, y: number }[] {
  ...
  private calcolaDistanza(p1: { x: number, y: number }, p2: { x: number, y: number }): number {
